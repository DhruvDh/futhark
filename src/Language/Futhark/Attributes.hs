...
typeDimNamesPos (Scalar (Record fs)) = mconcat $ map typeDimNamesPos $ M.elems fs
typeDimNamesPos (Scalar (TypeVar _ _ _ targs)) = mconcat $ map onTypeArg targs
  where onTypeArg (TypeArgDim (NamedDim d) _) = S.singleton $ qualLeaf d
        onTypeArg (TypeArgDim _ _) = mempty
        onTypeArg (TypeArgType t _) = typeDimNamesPos t
typeDimNamesPos t = typeDimNames t
...
intrinsics = M.fromList $ zipWith namify [10..] $
             ...
             [("flatten", IntrinsicPolyFun [tp_a]
              ...
              ("arbitrary", IntrinsicPolyFun [tp_a] [Scalar $ Record mempty] $ Scalar t_a)]
